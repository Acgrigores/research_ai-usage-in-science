...
#hello
class ScientificReports(Journal_ABC):
    def __init__(self) -> None:
        self.journalName: str = "Nature"
        self.paperURLTemplate: Template = Template(
            template="https://www.nature.com/articles/{paperID}"
        )  # noqa: E501
        self.searchURLTemplate: Template = Template(
            template="https://www.nature.com/search?q={query}&journal=srep&article_type=research&date_range={year}-{year}&order=relevance&page={page}"  # noqa: E501
        )
        ...
        with Bar(f"Conducting search for {query} in {year}...", max=1) as bar:
            while True:
                ...
                with open("test.html", "w") as htmlFile:
                    htmlFile.write(resp.content.decode(errors="ignore"))
                    htmlFile.close()
                ...
                if page == 1:
                    ...
                    # Find the last page number from the pagination buttons
                    pagination_items = soup.find_all('li', {'class': 'c-pagination__item'})
                    print(resp.status_code)
                    with open("hi.html", 'w', encoding='utf-8') as file:
                        file.write(str(soup))

                    if pagination_items:
                        try:
                            last_page_num = max(int(item['data-page']) for item in pagination_items if 'data-page' in item.attrs)
                            maxPage = last_page_num
                            bar.max = maxPage
                            bar.update()
                        except ValueError:
                            print("Error: Could not determine last page number from pagination items.")
                            maxPage = 1  # Default to 1 if there's a problem determining the last page number
                    else:
                        print("Error: Pagination items not found in the HTML.")
                        maxPage = 1  # Default to 1 if no pagination items are found
